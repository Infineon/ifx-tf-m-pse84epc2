                                        ...
                                        const ifx_mpc_raw_region_config_t* mpc_reg_cfg)
{
    ...
    for (uint32_t pc = 0U; pc < MPC_PC_NR; pc++) {
        ...
        for (uint32_t block_offset = first_offset; block_offset < last_offset; block_offset+=block_size) {
...
FIH_RET_TYPE(enum tfm_hal_status_t) ifx_mpc_init_cfg(void)
{
    ...
    */
    ...
    for (uint32_t idx = 0UL; idx < cy_response_mpcs_count; idx++) {
        ...
        TFM_COVERITY_BLOCK_END(MISRA_C_2023_Rule_10_4)
            cy_en_mpc_status_t status = Cy_Mpc_SetViolationResponse(cy_response_mpcs[idx].base, cy_response_mpcs[idx].response);
                                                         ...
                                                         uint32_t access_type)
{
    ...
    TFM_COVERITY_DEVIATE_LINE(MISRA_C_2023_Rule_10_1, "external macro IS_NS_AGENT_MAILBOX interprets non-boolean type as a boolean")
                            ((IS_NS_AGENT_MAILBOX(p_info->p_ldinfo)) && ifx_is_ns_cpu_internal_memory(base, size)) ||
    ...
    TFM_COVERITY_DEVIATE_LINE(MISRA_C_2023_Rule_10_1, "external macro IS_NS_AGENT_TZ interprets non-boolean type as a boolean")
                            ((IS_NS_AGENT_TZ(p_info->p_ldinfo)) && ifx_is_ns_cpu_internal_memory_remapped(base, size)) ||
    ...
    fih_int is_secure2 = ((access_type_2 & TFM_HAL_ACCESS_NS) == 0U) ? FIH_SUCCESS : FIH_FAILURE;
    TFM_COVERITY_DEVIATE_BLOCK(MISRA_C_2023_Rule_10_4, "Cannot change types due to Fault injection architecture")
    TFM_COVERITY_DEVIATE_LINE(MISRA_C_2023_Rule_10_1, "Cannot change equal logic due to Fault injection architecture and define fih_eq")
    ...
    for (uint32_t block_offset = first_block_idx * block_size; block_offset < last_offset; block_offset += block_size) {
        cy_stc_mpc_rot_block_attr_t mpc_settings;
        cy_stc_mpc_block_attr_t mpc_settings_no_rot;

        if (rot_config) {
            if (Cy_Mpc_GetRotBlockAttr(mem_region_cfg->mpc, (cy_en_mpc_prot_context_t)fih_int_decode(pc), block_offset, &mpc_settings) != CY_MPC_SUCCESS) {
                FIH_RET(fih_int_encode(TFM_HAL_ERROR_MEM_FAULT));
            }
        } else {
            if (Cy_Mpc_GetBlockAttr(mem_region_cfg->mpc, block_offset, &mpc_settings_no_rot) != CY_MPC_SUCCESS) {
                FIH_RET(fih_int_encode(TFM_HAL_ERROR_MEM_FAULT));
            }
        }
        ...
        if (rot_config) {
            ...
            if (Cy_Mpc_GetRotBlockAttr(mem_region_cfg->mpc, (cy_en_mpc_prot_context_t)fih_int_decode(pc), block_offset, &mpc_settings2) != CY_MPC_SUCCESS) {
                FIH_RET(fih_int_encode(TFM_HAL_ERROR_MEM_FAULT));
            }
            ...
            if (Cy_Mpc_GetBlockAttr(mem_region_cfg->mpc, block_offset, &mpc_settings_no_rot2) != CY_MPC_SUCCESS) {
                FIH_RET(fih_int_encode(TFM_HAL_ERROR_MEM_FAULT));
            }
