                                      ...
                                      const uint32_t                ppc_pcmask)
{
    ...
    if (pc_mask & lock_mask) {
        if (attr.secAttribute != ppc_attribute->secAttribute) {
            SPMLOG_ERRMSGVAL("PPC config cannot be set (secAttribute when locked PC has access) for region ", (uint32_t)ppc_region);
        ...
        if (attr.privAttribute != ppc_attribute->privAttribute) {
            SPMLOG_ERRMSGVAL("PPC config cannot be set (privAttribute when locked PC has access) for region ", (uint32_t)ppc_region);
            SPMLOG_ERRMSGVAL("pc_mask ", pc_mask);
            SPMLOG_ERRMSGVAL("lock_mask ", lock_mask);
            SPMLOG_ERRMSGVAL("current privAttribute ", (uint32_t)attr.privAttribute);
            tfm_core_panic();
        }
    }
    ...
    if ((pc_mask ^ ppc_pcmask) & lock_mask) {
        SPMLOG_ERRMSGVAL("PPC config cannot be set (PC mask for locked PC) for region ", (uint32_t)ppc_region);
        SPMLOG_ERRMSGVAL("pc_mask ", pc_mask);
        SPMLOG_ERRMSGVAL("lock_mask ", lock_mask);
        tfm_core_panic();
    }
    ...
    if (((pc_mask & lock_mask) != 0U) && (((pc_mask ^ ppc_pcmask) & ppc_pcmask) != 0U)) {
        SPMLOG_ERRMSGVAL("PPC config cannot be set (grant access when locked PC has access) for region ", (uint32_t)ppc_region);
        SPMLOG_ERRMSGVAL("pc_mask ", pc_mask);
        SPMLOG_ERRMSGVAL("lock_mask ", lock_mask);
        tfm_core_panic();
    }
