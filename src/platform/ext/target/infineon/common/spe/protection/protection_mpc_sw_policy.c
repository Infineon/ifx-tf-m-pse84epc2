                                                         ...
                                                         uint32_t access_type)
{
    ...
    if (ifx_is_region_inside_other(base_s, base_s + size, CY_SRAM0_BASE_S, CY_SRAM0_BASE_S + CY_SRAM0_SIZE)) {
        /* Handle SRAM0 MPC - SRAM0 MPCs are stored in indexes
         * 0 -> SFLASH->N_RAM_MPC */
        mpc_start_idx = 0;
        mpc_end_idx = mpc_policy->n_ram_mpc;
        mpc_base_addr = CY_SRAM0_BASE_S;
    } else if (ifx_is_region_inside_other(base_s, base_s + size, CY_FLASH_BASE_S, CY_FLASH_BASE_S + CY_FLASH_SIZE)) {
    ...
    for (uint32_t mpc_idx = mpc_start_idx; mpc_idx < mpc_end_idx; mpc_idx++) {
        ...
        if (ifx_is_region_inside_other(base_s, base_s + size, mpc_region_start_addr, mpc_region_end_addr)) {
