...
FIH_RET_TYPE(enum tfm_hal_status_t) ifx_mpc_verify_static_boundaries(void)
{
    ...
    for (uint32_t idx = 0UL; idx < cy_mpc_unified_config_count; idx++) {
        if (!IFX_MPC_IS_EXTERNAL(cy_mpc_unified_config[idx].base))) {
            uint32_t response = cy_mpc_unified_config[idx].response ? 1u : 0u;
            if (_FLD2VAL(RAMC_MPC_CFG_RESPONSE, cy_mpc_unified_config[idx].base->CFG) != response) {
                FIH_RET(fih_int_encode(TFM_HAL_ERROR_GENERIC));
            }
        }
    }
                                                         ...
                                                         uint32_t access_type)
{
    ...
    for (uint32_t idx = first_block_idx; idx < last_block_idx; idx++) {
        ...
        if (rot_config) {
            ...
            if (Cy_Mpc_GetBlockAttr(mem_region_cfg->mpc, idx, &mpc_settings_no_rot) != CY_MPC_SUCCESS) {
                FIH_RET(fih_int_encode(TFM_HAL_ERROR_MEM_FAULT));
            }
        }
        ...
        if (rot_config) {
            ...
            if (Cy_Mpc_GetBlockAttr(mem_region_cfg->mpc, idx, &mpc_settings_no_rot2) != CY_MPC_SUCCESS) {
                FIH_RET(fih_int_encode(TFM_HAL_ERROR_MEM_FAULT));
            }
