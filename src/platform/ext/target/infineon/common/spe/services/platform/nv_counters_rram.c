...
#define IFX_TFM_NV_COUNTERS_BASE                   (0UL) /* Address of NV counters block, it is equal 0 as relative are used */
#define IFX_TFM_NV_COUNTERS_COUNTER_AREA_OFFSET    (0UL) /* Relative offset of counters area in NV counters area */
/* Relative offset of backup area in NV counters area */
#define IFX_TFM_NV_COUNTERS_BACKUP_AREA_OFFSET     (IFX_TFM_NV_COUNTERS_COUNTER_AREA_OFFSET + ((PLAT_NV_COUNTER_MAX) * sizeof(ifx_rram_counter_t)))
...
#define IFX_TFM_NV_COUNTERS_NVM_INIT_DONE_FLAG_OFFSET    (IFX_ALIGN_UP_TO((IFX_TFM_NV_COUNTERS_BACKUP_AREA_OFFSET + \
                                                                IFX_TFM_NV_COUNTERS_BACKUP_AREA_SIZE), IFX_TFM_NV_COUNTERS_SECTOR_SIZE))
...
#define IFX_TFM_NV_COUNTERS_EXPECTED_AREA_SIZE           (IFX_ALIGN_UP_TO((sizeof(ifx_rram_counter_t) * (2U * ((uint32_t)PLAT_NV_COUNTER_MAX))), \
                                                                       IFX_TFM_NV_COUNTERS_SECTOR_SIZE) + IFX_TFM_NV_COUNTERS_SECTOR_SIZE)
    ...
    for (uint32_t counter_idx = 0UL; counter_idx < (uint32_t)PLAT_NV_COUNTER_MAX; counter_idx++) {
        ...
        if (status_backup == TFM_PLAT_ERR_SUCCESS) {
            ...
            if (status_counter == TFM_PLAT_ERR_SUCCESS) {
                ...
                /* If both counter and its backup have wrong checksums it means some error or not initialized counters */
                return TFM_PLAT_ERR_SYSTEM_ERR;
            }
        }
    }
...
enum tfm_plat_err_t tfm_plat_init_nv_counter(void)
{
    ...
    TFM_COVERITY_DEVIATE_LINE(MISRA_C_2023_Rule_2_2, "This code is added to perform compile time check")
    IFX_ASSERT((uint32_t)IFX_TFM_NV_COUNTERS_AREA_SIZE >= (uint32_t)IFX_TFM_NV_COUNTERS_EXPECTED_AREA_SIZE);
